...
export default function Admin() {
  ...
  const createPost = async () => {
    if (editorRef.current && title && tags.length > 0) {
      ...
      try {
        putBlog(title, "Sanchay Javeria", content, tags)
          .then(blogId => {
            // email subscribers
            fetch("/api/mail", {
              method: "POST",
  ...
  const notepad: JSX.Element = (
    <div className="container flex flex-col items-center mx-auto mt-16 max-w-7xl px-8 py-12">
      ...
      <div className="w-full py-2">
        ...
        init={{
          plugins: 'codesample anchor autolink link lists image code advcode mergetags wordcount',
          toolbar: 'undo redo | blocks | bold italic strikethrough backcolor | mergetags | link image | align bullist numlist | codesample code',
          height: 900,
