...
function Comment({ comment, currentUser }: CProps) {
  ...
  const { collapsed: collapsedButton, toggleCollapse: toggleCollapseButton } =
    useCollapseState(true);
    ...
    <article className="mt-8">
      ...
      {!collapsed &&
        ...
        replies
          ...
          .map((reply) => (
            <article
              key={`${comment.id}-${reply.id || Math.floor(Math.random() * 100)}`}
    ...
    if (currentUser) {
      ...
      if (liked) {
        ...
        if (like) {
          deleteLike(like.id);
          setLikes((prevLikes) =>
            prevLikes.filter((l) => l.name !== currentUser.email),
          );
        }
      } else {
        const newLike: ILike = {
